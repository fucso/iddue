#!/usr/bin/env node
// idd issue YAML から branch 情報を抽出する
// 外部依存なし - Node.js 組み込みモジュールのみ使用
//
// Usage: node get-branch-info.js <issue_number>
// Output: JSON { taskBranch, setupBranch }
//
//   taskBranch  - 実装ブランチ名（issue.linked_pr.branch が存在すればその値、なければ "idd/{issue_number}"）
...
const yamlPath = `.idd/issue/${issueNumber}.yaml`;
let content;
try {
...
// 対象: idd:fetch-issue が生成する 2-space インデント・ダブルクォート文字列形式。
...
const taskBranch     = existingBranch ?? `idd/${issueNumber}`;
