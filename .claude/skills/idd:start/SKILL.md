...
description: |
  ...
  GitHub Issue からコンテキストを収集し、idd:judging-ready-to-implementation で
...
`idd:fetch-issue` スキルを実行して Issue コンテキストを収集する。
...
**エラー:** `idd:fetch-issue` がエラーを返した場合はその内容を表示して終了する。
...
```bash
node .claude/skills/idd:start/scripts/get-branch-info.js {issue_number}
```
...
- `sub_issues` が空でない場合（例: `[124, 125, 126]`）:
  ...
  - **Yes** → `/idd:start:orchestrate {issue_number}` へデリゲートして終了
  - **No** → 通常フローを終了（実装しない）
...
`idd:judging-ready-to-implementation` スキルを実行して Issue の実装可能性を評価する。
Phase 1 で `force:true` 付きの fetch が済んでいるため、内部の `idd:fetch-issue` 呼び出し（force なし）はキャッシュを返し GitHub fetch は発生しない。

```
/idd:judging-ready-to-implementation {issue_number}
```
...
```bash
bash .claude/skills/worktree-development/scripts/setup.sh "idd/{issue_number}" {setupBranch}
```
...
```bash
bash .claude/skills/idd:start/scripts/quality-check.sh "idd/{issue_number}"
```
...
```bash
bash .claude/skills/idd:start/scripts/finalize.sh "idd/{issue_number}" "{commit_message}" {taskBranch}
```
...
finalize.sh 完了後、`.idd/issue/{issue_number}.yaml` の `issue.linked_pr.branch` を更新する。
...
```bash
bash .claude/skills/idd:start/scripts/create-pr.sh \
  "{task_branch}" \
...
PR 作成後（または既存 PR が見つかった場合）、`.idd/issue/{issue_number}.yaml` の `issue.linked_pr` を完全な情報に更新する。
...
| エラー | 対応 |
|--------|------|
| `idd:fetch-issue` がエラーを返した | エラー内容を表示して終了（Issue 不在・クローズ済み・依存 Issue 未クローズ・親ブランチ取得不可を含む） |
...
| 項目 | 形式 | 例 |
|------|------|-----|
| 作業環境名 | `idd/{issue_number}` | `idd/103` |
| worktree パス | `.worktree/idd/{issue_number}` | `.worktree/idd/103` |
| 実装ブランチ | `idd/{issue_number}` | `idd/103` |
