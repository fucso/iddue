---
name: idd:pr:check-feedback-resolution
description: |
...
# `/idd:pr:check-feedback-resolution` — PR フィードバック解決確認

## 使い方

```
/idd:pr:check-feedback-resolution {Issue 番号}
例: /idd:pr:check-feedback-resolution 123
```
...
| 種別 | 判定方法 | 処理 |
|------|---------|------|
| **異議スレッド** | `<!-- idd:pr:review -->` 付きコメントへのユーザー返信 | Phase 2: ユーザーと議論して解消 |
| **コード修正待ちスレッド** | 上記以外の未解決スレッド | Phase 3: コード変更で解消されているか確認 |

| チェック対象 | 参照元 |
|-------------|--------|
| **コメント後の差分** | `get-diff.sh`（`original_commit_oid` から `headRefOid` まで） |
| **最新のコード全体** | worktree（`.worktree/idd-review/{parent}/`）を `Read` で参照 |
...
`unresolved_threads` から、このスキルが過去に投稿した結果コメント（本文が `<!-- idd:pr:check-feedback-resolution -->` で始まるもの）を検出する。
...
前回のレビューコメント（`<!-- idd:pr:review -->` マーカー付き）にユーザーが返信を追加している場合、
...
- `comments.nodes` のいずれかの `body` に `<!-- idd:pr:review -->` が含まれる
...
--- {path} ---
[AI レビューコメント]
{<!-- idd:pr:review --> を含むコメント本文（先頭 300 字）}
...
- `comment_id`: `<!-- idd:pr:review -->` を含むコメントの `databaseId`
...
必要であれば Issue を更新する（`/idd:issue:revise`）。
...
```
Read ツールで .worktree/idd-review/{parent}/{path} を参照
```
