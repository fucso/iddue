...
# `/iddue:open:concrete` — 具体的な改修ポイント起票
...
```
/iddue:open:concrete
/iddue:open:concrete UserForm の email バリデーションを修正したい
```
...
```
✅ /iddue:open:concrete 完了

- Issue: [#{NNN}] {タイトル}
  URL: {URL}

## 次のステップ
実装を開始する: /iddue:start {NNN}
```

判定 NG のまま中断した場合：

```
⚠️ /iddue:open:concrete 中断

- Issue: [#{NNN}] {タイトル}
  URL: {URL}

実装可能性の判定が NG のまま終了しました。
Issue に不足内容を追記してから /iddue:start {NNN} で実装を開始してください。
```
...
作成したオーダーシートの内容をユーザーに確認する。
確認後、`iddue:create-issue` スキルで親 Issue を作成する。
...
各 Sub Issue について create オーダーシートを作成し `iddue:create-issue` で登録する（`parent_issue` に親 Issue 番号を指定）。

ファイル名: `.iddue/orders/{YYYYMMDD-HHMMSS}-create-sub{n}.yaml`
...
`iddue:setup-orchestration` スキルでメイン Issue ブランチ・config.yaml・メイン Issue PR の作成と
...
- 親 Issue: [#{NNN}] {タイトル}
  URL: {URL}
- メイン Issue PR: {PR URL}（iddue:setup-orchestration 完了時に表示）

## 次のステップ
オーケストレーター実装を開始する: /iddue:start {NNN}
```
