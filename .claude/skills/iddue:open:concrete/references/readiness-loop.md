...
```
/iddue:judging-ready-to-implementation {issue番号}
```
...
```yaml
# .iddue/orders/{YYYYMMDD-HHMMSS}-update.yaml
type: update
...
確認後、`iddue:update-issue` スキルで Issue を更新する：

```
/iddue:update-issue .iddue/orders/{ファイル名}
```

更新後、再判定する：

```
/iddue:judging-ready-to-implementation {issue番号} refresh:true
```
