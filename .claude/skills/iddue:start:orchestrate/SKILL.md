---
name: iddue:start:orchestrate
description: |
  サブ Issue を持つ親 Issue のオーケストレーターモード実装スキル。
  iddue:setup-orchestration で準備済みの config.yaml と feature branch をベースに、
  各サブ Issue を並列ワーカープロセスで実装し、完了次第メイン Issue ブランチへマージする。
argument-hint: "{親 Issue 番号}"
---

# `/iddue:start:orchestrate` — オーケストレーターモード実装
...
```
/iddue:start:orchestrate {親 Issue 番号}
例: /iddue:start:orchestrate 123
```
...
```bash
bash .claude/skills/iddue:start:orchestrate/scripts/orchestrate.sh {parent}
```
...
```bash
bash .claude/skills/iddue:start:orchestrate/scripts/check-completion.sh {parent}
```
...
```bash
REPO=$(bash .claude/skills/github-pr/scripts/get-repo.sh)
PR_URL=$(gh pr list --head "iddue/{parent}" --repo "${REPO}" --json url --jq '.[0].url')
gh pr ready "$PR_URL"
...
node .claude/skills/iddue:start:orchestrate/scripts/tasks.js skip {sub} "crashed-skipped"
