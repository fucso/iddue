---
name: idd:setup-orchestration
description: |
  親 Issue と Sub Issue が GitHub 上に存在する状態で、
   /idd:start:orchestrate による並列実装を実行するための前提情報を整備する。
argument-hint: "{親 Issue 番号}"
---

# `/idd:setup-orchestration` — 並列実装前提整備

親 Issue と Sub Issue が GitHub 上に起票済みであることを前提に、
オーケストレーターモードによる並列実装を開始するための情報を整備する。

**このスキルのゴール:**
- メイン Issue ブランチ `idd/{parent}` の作成と push
- `.idd/orchestration/config.yaml` の生成
- メイン Issue PR の作成（ドラフト）
- 各サブ Issue YAML に `parent.linked_pr.branch` を設定

完了後は `/idd:start {parent}` でオーケストレーター実装を開始できる。

## 使い方

```
/idd:setup-orchestration {親 Issue 番号}
例: /idd:setup-orchestration 123
```

**実行タイミング:** 親 Issue と Sub Issue が GitHub 上に起票済みで、/idd:start:orchestrate による並列実装を開始したいとき。
...
```
/idd:fetch-issue {parent}
```

`.idd/issue/{parent}.yaml` の `sub_issues` フィールドを確認する。
...
```
/idd:fetch-issue {sub1}
/idd:fetch-issue {sub2}
...
```
...
```bash
bash .claude/skills/worktree-development/scripts/setup.sh "idd/{parent}" {BASE_BRANCH}
```
...
```
.worktree/idd/{parent}/.idd/orchestration/config.yaml
```

内容は Phase 1 で idd:fetch-issue から返された YAML を使用する。
（`.idd/issue/` は gitignore 対象のため worktree には存在しないが、Phase 1 で既に取得済み）

スキーマの詳細は `.claude/skills/idd:orchestration-development/templates/config-yaml.md` を参照。

```yaml
parent_issue: {parent}
main_issue_branch: "idd/{parent}"
...
第3引数で `idd/{parent}` ブランチを worktree 内に作成してからコミットする。

**Step 5: プッシュ**

```bash
bash .claude/skills/worktree-development/scripts/push.sh "idd/{parent}" "idd/{parent}"
```

**Step 6: クリーンアップ**

```bash
bash .claude/skills/worktree-development/scripts/cleanup.sh "idd/{parent}"
```
...
```bash
BODY_FILE=$(mktemp /tmp/idd-pr-body.XXXXXX.md)
cat > "$BODY_FILE" << 'EOF'
Closes #{parent}
...
各サブ Issue の `.idd/issue/{sub}.yaml` を更新して `parent.linked_pr.branch` を設定する。

`idd:issue-yaml` スキルを参照してフィールドを更新する：
...
`.idd/issue/{parent}.yaml` の `issue.linked_pr` を更新：
...
```
✅ /idd:setup-orchestration 完了

**親 Issue:** #{parent} {タイトル}
**メイン Issue ブランチ:** idd/{parent}
**メイン Issue PR:** {PR URL}
