...
# `/idd:pr:review` — IDD PR AI レビュー

## 使い方

```
/idd:pr:review {Issue 番号}
例: /idd:pr:review 123
```

**前提条件:**
- `idd/{parent}` ブランチの PR が存在すること
- **オーケストレーション PR の場合**: `.idd/orchestration/status.yaml` で全サブ Issue が `complete` であること
...
| **実装後のコード全体** | worktree（`.worktree/idd-review/{parent}/`） | コードの完全なコンテキスト・品質確認 |
| **オーケストレーション成果物** | worktree 内の `.idd/orchestration/` | status.yaml・implement.md（オーケストレーション PR のみ） |
...
```
Phase 1: フィードバック解決確認（idd:pr:check-feedback-resolution を呼び出す）
...
観点⑤コード品質 → CRITICAL? → PR にコメント投稿して終了（cleanup）
         ↓ OK
レビュー OK コメント投稿 + ラベル付与（idd: review ok）
...
`idd:pr:check-feedback-resolution` スキルを呼び出す。

このスキルは以下を処理する：
- 前回のレビューコメント（`<!-- idd:pr:review -->` マーカー付き）へのユーザー返信（異議コメント）を検出し、ユーザーと議論して解消する
...
```
/idd:fetch-issue {parent} force:true
```
...
```
environment_name: idd-review/{parent}
base branch:      idd/{parent}
worktree path:    .worktree/idd-review/{parent}/
```

**実行コマンド:**
```bash
bash .claude/skills/worktree-development/scripts/setup.sh "idd-review/{parent}" "idd/{parent}"
```

以降、**PR ブランチのコード全体**は `.worktree/idd-review/{parent}/` 配下のファイルを `Read` ツールで参照する。
...
| ファイル | パス |
|---------|------|
| **status.yaml** | `.worktree/idd-review/{parent}/.idd/orchestration/status.yaml` |
| 各サブ implement.md | `.worktree/idd-review/{parent}/.idd/orchestration/reports/{sub}/implement.md` |
| 親 Issue YAML（任意） | `.worktree/idd-review/{parent}/.idd/issue/{parent}.yaml` |
...
- **変更後の完全なコード**: `Read` ツールで `.worktree/idd-review/{parent}/{ファイルパス}` を参照
...
このスキルが投稿するすべてのレビューコメント（本文・インライン）の先頭に `<!-- idd:pr:review -->` を付与する。
これにより次回レビュー時に `idd:pr:check-feedback-resolution` がユーザー返信（異議コメント）を検出できる。

### 4.1 REQUEST_CHANGES（CRITICAL あり）

問題箇所を diff の行に紐づけられる場合は inline コメントとして投稿する。
インラインコメントの `body` にも `<!-- idd:pr:review -->` を付与すること。

```bash
# インラインコメントを JSON ファイルに書き出す
cat > /tmp/review-comments.json << 'EOF'
[{"path": "{変更ファイルパス}", "line": {問題の行番号}, "body": "<!-- idd:pr:review -->\n{インラインコメント本文}"}]
EOF
...
コメント投稿後、PR に `idd: review ok` ラベルを付与する：

```bash
bash .claude/skills/github-pr/scripts/add-label.sh -h {pr_info.number} "idd: review ok"
```
...
```bash
bash .claude/skills/worktree-development/scripts/cleanup.sh "idd-review/{parent}"
```
