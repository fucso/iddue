...
#   3. .claude/idd-hooks/get-ci-failure-details.sh が存在すれば呼び出し
#
...
FAILED_CHECKS_FILE=$(mktemp /tmp/idd-failed-checks.XXXXXX.json)
ENRICHED_FILE=$(mktemp /tmp/idd-enriched-checks.XXXXXX.json)
...
HOOK=".claude/idd-hooks/get-ci-failure-details.sh"
if [ -f "${HOOK}" ]; then
  echo ""
  echo "---"
