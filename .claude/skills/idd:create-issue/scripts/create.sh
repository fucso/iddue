#!/bin/bash
# idd:create-issue/scripts/create.sh
...
# Arguments:
#   title               Issue タイトル
#   labels              ラベルのカンマ区切り文字列（例: "idd,level:concrete"）
...
BODY_FILE=$(mktemp /tmp/idd-issue-body.XXXXXX.md)
...
for label in "${LABEL_LIST[@]}"; do
  bash "${SCRIPT_DIR}/../../idd/scripts/add-label.sh" "$ISSUE_NUMBER" "$label"
done
