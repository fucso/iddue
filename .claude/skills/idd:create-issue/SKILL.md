...
# `idd:create-issue` — Issue 作成

オーダーシート（`.idd/orders/*.yaml`）を受け取り、Issue タイトル・本文を組み立てて GitHub Issue を作成する。
...
```
/idd:create-issue {order_sheet_path}
```
...
オーダーシートのすべての内容を把握した上で、`idd/templates/issue.md` の構造に従い本文を組み立てる。
...
```bash
bash .claude/skills/idd:create-issue/scripts/create.sh \
  "{title}" \
  "{labels をカンマ区切りで結合: 例 "idd,level:concrete"}" \
...
`idd:issue-yaml` のスキーマ・記述原則に従い `.idd/issue/{issue_number}.yaml` を書き出す。
...
`idd:issue-yaml` の「親 Issue YAML への sub_issues 追記」ルールに従い実施する。
