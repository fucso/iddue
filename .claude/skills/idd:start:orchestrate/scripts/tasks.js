...
//
// データソース:
//   .idd/orchestration/config.yaml  (依存グラフ、read-only)
//   .idd/orchestration/status.yaml  (実行状態、read-write)
//
// サブコマンド:
...
function main() {
  ...
  const configFile = path.join(repoRoot, '.idd', 'orchestration', 'config.yaml');
  const statusFile = path.join(repoRoot, '.idd', 'orchestration', 'status.yaml');
