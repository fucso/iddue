...
# 終了コード:
#   0: 全タスク正常完了
#   99: エスカレーション要（.idd/orchestration/escalation.yaml に詳細）

set -uo pipefail

PARENT="${1:?Usage: $0 <parent-issue-number>}"
SCRIPTS="$(cd "$(dirname "$0")" && pwd)"
MAIN_BRANCH="idd/${PARENT}"
ESCALATION_FILE=".idd/orchestration/escalation.yaml"
...
if [ ! -f ".idd/orchestration/config.yaml" ]; then
  echo "Error: .idd/orchestration/config.yaml が見つかりません。idd:setup-orchestration を先に実行してください。" >&2
  exit 1
fi

if [ ! -f ".idd/orchestration/status.yaml" ]; then
    ...
    0)  # COMPLETED
      ...
      case $complete_exit in
        ...
        2) escalate "before-merge-hook-aborted" \
             "sub=#${sub}" \
             ".idd/orchestration/tasks/${sub}/worker.log" ;;
        3) escalate "merge-failed" \
             "sub=#${sub}" \
             ".idd/orchestration/tasks/${sub}/worker.log" ;;
        5) escalate "conflict-resolution-failed" \
             "sub=#${sub}" \
             ".idd/orchestration/reports/${sub}/conflict.md" ;;
...
if [ -n "$FAILED_TASK" ]; then
  escalate "task-failed-during-execution" \
    "failed_task=#${FAILED_TASK}, see .idd/orchestration/status.yaml"
fi
