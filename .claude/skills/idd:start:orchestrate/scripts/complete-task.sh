...
STATUS_FILE=".idd/orchestration/status.yaml"
...
if ! git merge --no-ff "${SUB_BRANCH}" -m "Merge sub-issue #${SUB} into ${MAIN_ISSUE_BRANCH}"; then
  ...
  if git diff --name-only --diff-filter=U | grep -q .; then
    ...
    CONFLICT_REPORT=".idd/orchestration/reports/${SUB}/conflict.md"
    mkdir -p "$(dirname "${CONFLICT_REPORT}")"

    # フォアグラウンドで起動（完了を待つ）
    if env -u CLAUDECODE claude -p \
         "/idd:orchestration-development:resolve-conflict ${SUB} ${PARENT}" \
         > "${CONFLICT_REPORT}" 2>&1; then
      # 解消成功: idd/<SUB> が parent の変更を含んだ状態に更新されているため再マージはクリーン
      git checkout "${MAIN_ISSUE_BRANCH}"
