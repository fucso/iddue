...
MAIN_ISSUE_BRANCH="idd/${PARENT_ISSUE_NUMBER}"

REPO_ROOT=$(git -C "$(dirname "$0")" rev-parse --show-toplevel)
cd "$REPO_ROOT"

TASKS_JS=".claude/skills/idd:start:orchestrate/scripts/tasks.js"
START_WORKER=".claude/skills/idd:start:orchestrate/scripts/start-worker.sh"
...
while IFS= read -r SUB; do
  ...
  LOG_DIR=".idd/orchestration/tasks/${SUB}"
  mkdir -p "$LOG_DIR"
  JUDGING_LOG="${LOG_DIR}/judging.log"

  echo "Checking readiness for #${SUB}..."

  env -u CLAUDECODE claude --no-session-persistence -p "/idd:judging-ready-to-implementation ${SUB}" > "$JUDGING_LOG" 2>&1 || true
  ...
  if grep -q "✅" "$JUDGING_LOG"; then
    ...
    STATUS_CHANGED=true
    STARTED_LINES="${STARTED_LINES}  #${SUB} ${TITLE} idd/${SUB}\n"
  else
