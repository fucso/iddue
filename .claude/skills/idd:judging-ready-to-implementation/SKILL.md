---
name: idd:judging-ready-to-implementation
description: |
...
# `idd:judging-ready-to-implementation` — 実装可能性の判定

> **⚠️ スコープ制約: 引数で指定した Issue 番号 1件のみを判定する。**
> 他の Issue YAML ファイルの読み込み・評価は禁止。`.idd/issue/` 配下の他ファイル、config.yaml、status.yaml にアクセスしてはならない。
...
判定基準は `idd/references/workflow.md` の「実装可能な Issue の目安」を具体化・ケース化したもの。
...
```
/idd:judging-ready-to-implementation {Issue 番号}
例: /idd:judging-ready-to-implementation 101

# Issue 更新後に最新の内容で判定する場合
/idd:judging-ready-to-implementation 101 refresh:true
```
...
```bash
bash .claude/skills/idd:judging-ready-to-implementation/scripts/check-label.sh {Issue 番号}
```
...
`idd:fetch-issue` スキルを実行して Issue コンテキストを取得する。
...
```
/idd:fetch-issue {Issue 番号} force:true
```
...
```bash
bash .claude/skills/idd:judging-ready-to-implementation/scripts/add-label.sh {Issue 番号}
```
