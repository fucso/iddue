...
# `iddue:fetch-issue` — Issue コンテキスト収集

GitHub Issue を読み取り、実装開始・実装可否判定に必要なコンテキストを `.iddue/issue/{number}.yaml` に書き出して YAML 内容を返す。
...
例: /iddue:fetch-issue 103

# キャッシュを無視して強制再取得する場合
/iddue:fetch-issue 103 force:true
```
...
`.iddue/issue/{Issue 番号}.yaml` が存在するかチェックする。
...
```bash
bash .claude/skills/iddue:fetch-issue/scripts/fetch.sh {Issue 番号}
```
...
```bash
bash .claude/skills/iddue:fetch-issue/scripts/get-pr-docs-diff.sh {linked_pr.number}
```
...
`iddue:issue-yaml` のスキーマ・記述原則に従い、Step 2〜4 の結果を `.iddue/issue/{Issue 番号}.yaml` に書き出す。
...
**書き出し手順:**
1. `.iddue/issue/` ディレクトリが存在しない場合は作成する（`mkdir -p .iddue/issue`）
2. YAML ファイルを書き出す
