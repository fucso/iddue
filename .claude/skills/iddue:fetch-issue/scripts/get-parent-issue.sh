#!/bin/bash
# 指定した Issue の親 Issue と紐づいた PR を取得する
#
# Usage: ./get-parent-issue.sh <issue_number>
#   issue_number: 対象 Issue 番号
#
# Output: JSON { number, title, state, url, linked_pr } or null（親がない場合）
#   linked_pr: { number, branch, url, state } or null
#
# Example: bash .claude/skills/iddue:fetch-issue/scripts/get-parent-issue.sh 103
