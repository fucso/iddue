---
name: iddue:update-issue
description: |
  オーダーシート YAML を受け取り GitHub Issue を更新する。
  現在の Issue 内容に変更を適用して本文を再構築し、変更経緯・概要をコメントとして投稿する。
  iddue:open:* スキルから呼び出される。直接呼び出しも可能。
argument-hint: "{order_sheet_path}"
---

# `iddue:update-issue` — Issue 更新

オーダーシート（`.iddue/orders/*.yaml`）を受け取り、GitHub Issue の本文を更新する。
...
```
/iddue:update-issue {order_sheet_path}
```
...
```bash
bash .claude/skills/iddue:update-issue/scripts/comment.sh "{issue_number}" << 'EOF'
{作成したコメント}
EOF
```
...
```bash
bash .claude/skills/iddue:update-issue/scripts/update.sh "{issue_number}" << 'EOF'
{更新後の本文}
EOF
```
...
```bash
bash .claude/skills/iddue:update-issue/scripts/label.sh \
  "{issue_number}" \
...
`iddue:issue-yaml` のスキーマ・記述原則に従い `.iddue/issue/{issue_number}.yaml` を更新する。
...
Updated: #{NNN}
URL: {URL}
YAML: .iddue/issue/{NNN}.yaml（更新した場合）/ スキップ（ファイルが存在しない場合）
```
