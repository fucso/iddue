...
name: idd:issue-yaml
description: |
  .idd/issue/{number}.yaml を作成・編集・参照する際に利用する知識スキル。
  YAML のスキーマ定義・記述原則・構造操作のルールを提供する。
user-invocable: false
---

# `idd:issue-yaml` — Issue YAML 構成ルール

`.idd/issue/{number}.yaml` のスキーマ・記述方針・構造操作ルールを定義する。
...
```
.idd/issue/{Issue 番号}.yaml
```

ディレクトリが存在しない場合は `mkdir -p .idd/issue` で作成してから書き出す。
...
Sub Issue 作成時、親 Issue 番号が判明しており、かつ `.idd/issue/{parent_issue_number}.yaml` が存在する場合：
