...
description: |
  ...
  以下の場合に使用:
  (1) 並列実装ワークフロー（iddue:setup-orchestration / iddue:start:orchestrate / iddue:start:worker）の仕組みを理解したいとき
...
| スキル | フェーズ | 担当 |
|--------|---------|------|
| `iddue:open:concrete` | 設計 | サブ Issue の分割案を決定・起票 |
| `iddue:setup-orchestration` | 設計 | メイン Issue ブランチ・config.yaml・メイン Issue PR の作成 |
| `iddue:start` | 実装 | サブ Issue 検出 → orchestrate への委譲 |
| `iddue:start:orchestrate` | 実装 | ワーカーのディスパッチ・監視・マージ |
| `iddue:start:worker` | 実装 | サブ Issue 単体の worktree 実装 |
