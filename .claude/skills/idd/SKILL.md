...
description: |
  ...
  (3) idd: プリフィックスのスキル、コマンドの利用、メンテナンス
...
| [references/hooks.md](references/hooks.md) | IDD フックスクリプト仕様（`.claude/idd-hooks/` へのカスタマイズポイント） |

## 並列実装ワークフロー

サブ Issue を持つ Issue をオーケストレーター/ワーカーパターンで並列実装する場合は、以下のスキルを参照する。

- [`idd:orchestration-development`](../idd:orchestration-development/SKILL.md) — 並列実装ワークフローの全体像・設計方針・ファイル構造
