...
`idd:setup-orchestration` が生成する read-only ファイル。依存グラフを定義する。

## ファイルパス

```
.idd/orchestration/config.yaml
```
...
main_issue_branch: "idd/123"         # メイン Issue ブランチ名
...
- `idd:start:orchestrate` は config.yaml を **読み取り専用** で扱う
...
`idd:setup-orchestration` の Phase 2 でエージェントが直接書き出し、
メイン Issue ブランチ（`idd/{parent}`）にコミットされる。
