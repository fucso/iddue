...
# `/iddue:issue:revise` — Issue 内容の修正・補完
...
```
/iddue:issue:revise {issue_number}
/iddue:issue:revise {issue_number} {brief_context}
```
...
```bash
bash .claude/skills/iddue:fetch-issue/scripts/get-issue.sh {issue_number}
```
...
```bash
bash .claude/skills/iddue:fetch-issue/scripts/get-sub-issues.sh {issue_number}
```
...
```bash
bash .claude/skills/iddue:fetch-issue/scripts/get-issue.sh {sub_issue_number}
```
...
`.iddue/orders/{YYYYMMDD-HHMMSS}-revise-{issue_number}.yaml` に保存する。

`labels.remove` には `ready to implementation` を常に含める（ラベルが付いていない場合も指定してよい。スクリプトが存在確認を行う）。

### 4.5 `iddue:update-issue` を実行する

```
/iddue:update-issue .iddue/orders/{YYYYMMDD-HHMMSS}-revise-{issue_number}.yaml
```
...
実装内容が確定したら /iddue:judging-ready-to-implementation で再判定してください。
```
